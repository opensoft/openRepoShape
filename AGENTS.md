...
`docs/cli.md` is the flag-level reference: every tool's own `--help`, and
every subcommand's, in one file — the ones you run from a checkout of this
standard, and the six that ship INTO a project and are run from there. Read it
when you know which act you are performing and need the exact spelling. It
does not replace this file: a flag documented there is not permission to pass
it. It is GENERATED by `scripts/render-cli-reference.py` and never
hand-edited; `tests/test_cli_reference.py` fails when the two disagree.
