...
# owns exactly three things, each of them something Python cannot do for
# itself before it is running: an INTERPRETER (3.9 or newer, under whichever
# name this machine has one); a CHECKOUT when the person has none, because
# `curl … | bash` leaves no file on disk at all and this script is then the
# whole of what they have; and the INVOCATION DIRECTORY, which that checkout
# would otherwise lose — handed over as `--into`, so the new project lands
# where the person was standing and not beside a temporary checkout in /tmp
# (#39).
#
...
# rather than trust the directory a person happened to be standing in.
is_shape_checkout() {
...
# value starting with `-` is an option to git rather than a repository. The
# `--` below guards the argv too; this is what names the fault instead of
# leaving a confusing git error.
case "$SHAPE_REPO" in
-*) die "OPENREPOSHAPE_REPO is '$SHAPE_REPO', which starts with '-': that is an option to \`git clone\`, not a repository. Set it to a URL or a path." ;;
esac
...
# A VALUE spelled `--keep-shape-checkout` sets the flag too: a known false
# positive, and a harmless one, because `checked_value` in Python refuses that
# run before anything exists and all this flag decides is whether a temporary
...
case "$SHAPE_REF" in
-*) die "--shape-ref is '$SHAPE_REF', which starts with '-': that is an option to \`git checkout\`, not a ref to check out. Pass a branch, tag or commit." ;;
esac
